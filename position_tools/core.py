...
def get_angle(back_LED, front_LED):
    ...
    """
    return np.arctan2(front_LED[:, 1] - back_LED[:, 1],
                      front_LED[:, 0] - back_LED[:, 0])
...
def interpolate_nan(position, time=None):
    ...
    for position_ind in range(n_position_dims):
        f = interpolate.interp1d(
            time[not_nan], position[not_nan, position_ind],
            bounds_error=False, kind='linear')
        interpolated_position.append(
            np.where(np.isfinite(position[:, position_ind]),
                     position[:, position_ind], f(time)))
...
def get_velocity(position, time=None, sigma=15, sampling_frequency=1):
    ...
    return gaussian_smooth(np.gradient(position, time, axis=0), sigma,
                           sampling_frequency, axis=0, truncate=8)


def get_speed(position, time=None, sigma=15, sampling_frequency=1):
    velocity = get_velocity(position, time=time, sigma=sigma,
