...
def make_video(
    ...
    # Convert to pixels
    back_LED = convert_to_pixels(np.asarray(back_LED), frame_size, cm_to_pixels)
    front_LED = convert_to_pixels(np.asarray(front_LED), frame_size, cm_to_pixels)

    head_orientation = np.asarray(head_orientation)
    head_orient_end = np.asarray(head_position) + RADIUS * np.stack(
        (np.cos(head_orientation), np.sin(head_orientation)), axis=1
    )

    head_position = convert_to_pixels(
        np.asarray(head_position), frame_size, cm_to_pixels
    )

    head_orient_start = head_position.copy()
    head_orient_end = convert_to_pixels(head_orient_end, frame_size, cm_to_pixels)
    ...
    for time_ind in tqdm(
        ...
        if is_grabbed:
            ...
            if ~np.any(np.isnan(head_orient_start[time_ind])) & ~np.any(
                np.isnan(head_orient_end[time_ind])
            ):
                cv2.arrowedLine(
                    frame,
